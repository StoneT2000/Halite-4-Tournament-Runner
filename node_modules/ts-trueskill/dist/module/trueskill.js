 ...
 */
export function calcDrawMargin(drawProbability, size, 
...
export class TrueSkill {
    ...
    rate(ratingGroups, ranks = null, weights = null, minDelta = 0.0001) {
        ...
        const teamPerfVars = Array.from({ length: groupSize }, (_, i) => i).map(() => new Variable());
        const teamDiffVars = Array.from({ length: groupSize - 1 }, (_, i) => i).map(() => new Variable());
