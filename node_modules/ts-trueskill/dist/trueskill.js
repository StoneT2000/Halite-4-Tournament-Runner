...
const factorgraph_1 = require("./factorgraph");
const quality_helper_1 = require("./quality-helper");
const rating_1 = require("./rating");
/**
 * Calculates a draw-margin from the given drawProbability
 */
function calcDrawMargin(drawProbability, size, 
...
class TrueSkill {
     ...
     */
    constructor(mu = 25, sigma, beta, tau, drawProbability = 0.1, guassian = new ts_gaussian_1.Gaussian(0, 1)) {
        this.mu = mu;
    ...
    rate(ratingGroups, ranks = null, weights = null, minDelta = 0.0001) {
        ...
        // Build factor graph
        const flattenRatings = lodash_1.flatten(sortedRatingGroups);
        const flattenWeights = lodash_1.flatten(sortedWeights);
        const size = flattenRatings.length;
        // Create variables
        const fill = Array.from({ length: size }, (_, i) => i);
        const ratingVars = fill.map(() => new factorgraph_1.Variable());
        const perfVars = fill.map(() => new factorgraph_1.Variable());
        const teamPerfVars = Array.from({ length: groupSize }, (_, i) => i).map(() => new factorgraph_1.Variable());
        const teamDiffVars = Array.from({ length: groupSize - 1 }, (_, i) => i).map(() => new factorgraph_1.Variable());
        ...
        for (let idx = 0; idx < teamSizes.length; idx++) {
            const group = layers
                ...
                .map((f) => new rating_1.Rating(f.v.mu, f.v.sigma));
        ...
        return unsorting.map(v => {
            return lodash_1.zipObject(keys[v[0]], v[1]);
        });
    }
    /**
    ...
    quality(ratingGroups, weights) {
        ...
        const flattenRatings = lodash_1.flatten(ratingGroups);
        const flattenWeights = lodash_1.flatten(newWeights);
        const { length } = flattenRatings;
        // A vector of all of the skill means
        const meanMatrix = mathjs_1.matrix(flattenRatings.map(r => [r.mu]));
        const varianceMatrix = quality_helper_1.createVarianceMatrix(flattenRatings, length, length);
        const rotatedAMatrix = quality_helper_1.createRotatedAMatrix(newRatingGroups, flattenWeights);
        const aMatrix = mathjs_1.transpose(rotatedAMatrix);
        // Match quality further derivation
        const modifiedRotatedAMatrix = rotatedAMatrix.map((value, _, __) => (this.beta ** 2) * value);
        const start = mathjs_1.multiply(mathjs_1.transpose(meanMatrix), aMatrix);
        const ata = mathjs_1.multiply(modifiedRotatedAMatrix, aMatrix);
        const atsa = mathjs_1.multiply(rotatedAMatrix, mathjs_1.multiply(varianceMatrix, aMatrix));
        const middle = mathjs_1.add(ata, atsa);
        const end = mathjs_1.multiply(rotatedAMatrix, meanMatrix);
        // Make result
        const eArg = mathjs_1.det(mathjs_1.multiply(mathjs_1.multiply([[-0.5]], mathjs_1.multiply(start, mathjs_1.inv(middle))), end));
        const sArg = mathjs_1.det(ata) / mathjs_1.det(middle);
        return mathjs_1.exp(eArg) * Math.sqrt(sArg);
    }
    /**
    ...
    winProbability(a, b) {
        ...
        const sumSigma = a.reduce((t, n) => (n.sigma ** 2) + t, 0) +
    ...
    _buildRatingLayer(ratingVars, flattenRatings) {
        const t = this.tau;
        return ratingVars.map((n, idx) => new factorgraph_1.PriorFactor(n, flattenRatings[idx], t));
    }
    _buildPerfLayer(ratingVars, perfVars) {
        const b = this.beta ** 2;
        return ratingVars.map((n, idx) => new factorgraph_1.LikelihoodFactor(n, perfVars[idx], b));
    }
 ...
 */
function rate_1vs1(rating1, rating2, drawn = false, minDelta = 0.0001, env = new TrueSkill()) {
 ...
 */
function quality_1vs1(rating1, rating2, env = new TrueSkill()) {
 ...
 */
function rate(ratingGroups, ranks, weights, minDelta = 0.0001, env = new TrueSkill()) {
    return env.rate(ratingGroups, ranks, weights, minDelta);
}
exports.rate = rate;
function winProbability(a, b, env = new TrueSkill()) {
 ...
 * environment.
 */
function quality(ratingGroups, weights, env = new TrueSkill()) {
 ...
 */
function expose(rating, env = new TrueSkill()) {
